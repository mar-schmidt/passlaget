...
`portal_private.teams` lagrar ett versionerat `PortalState` som JSONB per lag. Familj, barn, vuxna, evenemang, historik och ändringslogg hör därmed till en enda sammanhängande transaktion. Upplägget är avsiktligt enkelt för små lag: varje ändring skriver om lagets tillstånd och kräver rätt versionsnummer. Även aktivering av en mejlprenumeration höjer versionen, så att en samtidig publicering inte missar den nya mottagaren. Databasen skyddar fortfarande varje skrivning med versionskontroll. Evenemangsredigering skickar dessutom den version av evenemanget som formuläret öppnades med (`baseEvent`). Servern jämför ursprung, aktuellt evenemang och användarens ändringar: orelaterade laguppdateringar och SportAdmin-synkning blockerar inte sparandet. Skilda fält och platser kan sammanföras; motstridiga ändringar av samma fält eller bemanningsplats ger HTTP 409 med kod `event_conflict`. En ny föräldrabekräftelse behålls när platsen inte redigerats. En samtidigt publicerad, inställd eller borttagen version måste granskas på nytt.

Vid en kapplöpning mellan läsning och databasskrivning läser servern om och gör samma jämförelse igen, högst tre försök. Regler för aktiva familjer, aktuella kallelsesvar och högst ett bemanningspass kontrolleras mot senaste lagdata varje gång. Automatisk fördelning och publicering får fortsätta efter orelaterade uppdateringar men kräver oförändrat evenemangsunderlag. SportAdmin-kopplingen jämförs separat när den ändras och sparas fortfarande atomiskt med utkastet. Äldre klienter utan `baseEvent` och övriga kommandon behåller den tidigare globala versionskontrollen.

Vid en verklig konflikt finns användarens ändringar kvar i det öppna formuläret. Feltexten anger vilken uppgift som krockar, att ändringarna inte sparats och hur den senaste versionen hämtas. Om utkastet redan sparats men fördelning/publicering avbryts anges det separat.
...
Alla privata tabeller har RLS och saknar rättigheter och policyer för `anon`/`authenticated`. Schemat exponeras inte via Data API. RPC-funktionerna `portal_backend`, `portal_sportadmin` och `portal_admin_contacts` är `SECURITY INVOKER` och har uttryckligen återkallad `EXECUTE` för alla browserroller. Bara `service_role` får köra dem. Backendnyckeln stannar i Edge Functions. Begränsade kolumnrättigheter till `auth.users(id,email,deleted_at,banned_until,raw_user_meta_data)` används för att matcha administratörer, läsa visningsnamn och utesluta borttagna eller spärrade kontaktkonton. Metadata används endast för visning, aldrig för behörighetsbeslut.

Föräldrasidans **Kontakta Lagförälder** hämtar `admin_contacts` från Edge-funktionen när valrutan öppnas. Svaret innehåller endast namn och mejladress för det valda lagets aktuella administratörer. Namnet kommer från Supabase-kontots **Display name**, i första hand `raw_user_meta_data.name`, därefter `displayName`, `display_name`, `fullName` eller `full_name`. Saknas namn visas kontots mejladress. Föräldraregistret påverkar inte kontaktvalets namn. Listan sparas inte i lagets schema eller i webbläsarens beständiga lagring. Ändrade visningsnamn och nya eller borttagna adminmedlemskap slår därför igenom nästa gång valrutan öppnas. Varje kontakt har en egen `mailto:`-länk; portalen skickar inget mejl åt användaren.
...
## Importerade bekräftelser före första publicering

Ett administrativt importerat utkast kan innehålla redan bekräftade platser. Stödet aktiveras endast för evenemang med serverregistrerad `confirmationImport` (källa och registreringstid); övriga evenemang behåller tidigare beteende. Första publiceringen behåller dessa när platsen har familj, status `confirmed`, giltig `confirmedAt` och `confirmedRevision` som motsvarar platsens revision. Vid publiceringen sätts båda revisionerna till 1. Vanligt `save_event` kan inte skapa en sådan bekräftelse genom att skicka statusflaggor; nya platser blir fortsatt obekräftade. Ändras tid, plats, familj eller ansvarig vuxen återställs bekräftelsen enligt samma regler som för andra pass. Importens tidpunkt dokumenterar registreringen av befintliga bekräftelser, inte när de ursprungligen lämnades i källan.

För dessa importerade evenemang köas inga tilldelningsmejl för redan bekräftade pass. Sändningskontrollen stoppar också sådana tilldelningsmejl. Övriga evenemangs tilldelningsmejl påverkas inte. Påminnelser gäller fortsatt bara obekräftade pass. Ändrings- och avbokningsmejl behåller sina tidigare regler.
