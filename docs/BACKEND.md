...
Alla privata tabeller har RLS och saknar rättigheter och policyer för `anon`/`authenticated`. Schemat exponeras inte via Data API. RPC-funktionerna `portal_backend`, `portal_sportadmin` och `portal_admin_contacts` är `SECURITY INVOKER` och har uttryckligen återkallad `EXECUTE` för alla browserroller. Bara `service_role` får köra dem. Backendnyckeln stannar i Edge Functions. Begränsade kolumnrättigheter till `auth.users(id,email,deleted_at,banned_until)` används för att matcha administratörer och utesluta borttagna eller spärrade kontaktkonton.

Föräldrasidans **Kontakta Lagförälder** hämtar `admin_contacts` från Edge-funktionen när valrutan öppnas. Svaret innehåller endast namn och mejladress för det valda lagets aktuella administratörer. Namn hämtas från en aktiv föräldrapost med samma mejladress; annars visas kontots mejladress. Listan sparas inte i lagets schema eller i webbläsarens beständiga lagring. Nya eller borttagna adminmedlemskap slår därför igenom nästa gång valrutan öppnas. Varje kontakt har en egen `mailto:`-länk; portalen skickar inget mejl åt användaren.
