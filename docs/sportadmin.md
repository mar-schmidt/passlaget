...
3. Öppna **Spelare & föräldrar** och välj **Aktivera registersynkning**. Befintliga ID-kopplingar och entydiga namnträffar behålls. **Synka nu** läser både registret och kallelsesvaren; därefter sker uppdatering varje timme.
...
Här finns portalens enda spelarlista, föräldrakontakter, senaste registersynkning och knappen **Synka nu**. SportAdmin-sidan hanterar anslutningen. Varje spelare har separata chips för **AKTIV/AVSLUTAD**, **SYNKAD/MANUELL** och vid behov **UNDANTAGEN**. Syskon delar fortsatt familjens bemanningsansvar och passhistorik.

- **SYNKAD:** SportAdmins fullständiga register för det valda lagets aktiva period styr spelarens namn och medlemsstatus samt föräldrarnas namn, telefonnummer och mejladresser. Dessa uppgifter visas utan redigeringsfält i Passlaget. **Visa & bemanning** låter admin ändra familjens undantag från automatisk tilldelning och perioder då familjen inte kan delta.
- **MANUELL:** spelaren och föräldrarna sköts i Passlaget via **Lägg till spelare**. Manuella spelare påverkas inte när de saknas i SportAdmin.
...
- Om en manuell spelare senare finns med samma namn i SportAdmin visas en identitetsfråga under **Spelare & föräldrar**, med de båda posternas föräldrakontakter. **Ändra till synkad** behåller spelarens och familjens ID, tidigare pass, historik och undantag. **Inte samma person** används endast för två olika spelare; båda behålls och valet sparas för kommande synkningar. Det finns inget val att fortsätta manuellt för samma person. Matchningen använder fullständiga namn utan skillnad på versaler eller extra mellanslag. Andra spelare fortsätter synkas medan frågan väntar på svar. En väntande registerpost importeras inte dubbelt. Servern kontrollerar registret på nytt och sparar beslutet med versionskontroll.
