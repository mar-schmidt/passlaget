-- Backend-only state and queue. No parent or signed-in browser may access raw rows.
...
-- Recovery can look up only an existing member by current Auth email.
grant usage on schema auth to service_role;
grant select (id, email) on auth.users to service_role;

create table portal_private.teams (
  id text primary key,
  slug text unique not null check (slug ~ '^[a-z0-9][a-z0-9-]{0,79}$'),
  version integer not null default 0 check (version >= 0),
  state jsonb not null check (jsonb_typeof(state) = 'object'),
  updated_at timestamptz not null default now(),
  check ((state #>> '{team,id}') is not null and state #>> '{team,id}' = id),
  check ((state #>> '{team,slug}') is not null and state #>> '{team,slug}' = slug),
  check ((state ->> 'version') is not null and (state ->> 'version')::integer = version)
);
create table portal_private.admin_memberships (
  team_id text not null references portal_private.teams(id) on delete cascade,
  user_id uuid not null references auth.users(id) on delete cascade,
  primary key (team_id, user_id)
);
create table portal_private.subscriptions (
  id uuid primary key default gen_random_uuid(),
  team_id text not null references portal_private.teams(id) on delete cascade,
  family_id text not null,
  adult_id text,
  scope text not null check (scope in ('family','adult')),
  ...
  check (scope <> 'adult' or adult_id is not null)
);
create unique index subscriptions_identity on portal_private.subscriptions
  (team_id, family_id, coalesce(adult_id, ''), scope, email);
create unique index subscriptions_verify_token on portal_private.subscriptions(verify_hash) where verify_hash is not null;

create table portal_private.outbox (
  id uuid primary key default gen_random_uuid(),
  team_id text references portal_private.teams(id) on delete cascade,
  subscription_id uuid references portal_private.subscriptions(id) on delete cascade,
  kind text not null check (kind in ('verify','recovery','assignment','changed','cancelled','reminder')),
  dedupe_key text not null unique,
  recipient text not null,
  subject text not null,
  payload jsonb not null default '{}'::jsonb,
  status text not null default 'queued' check (status in ('queued','leased','sent','failed','uncertain','suppressed')),
  ...
  last_error text,
  last_ack_token uuid,
  last_ack_outcome text
);
create index outbox_due on portal_private.outbox(priority, due_at) where status = 'queued';
create index outbox_team on portal_private.outbox(team_id, created_at desc);
create table portal_private.mail_resolutions (
  id uuid primary key default gen_random_uuid(),
  team_id text not null references portal_private.teams(id),
  mail_id uuid not null references portal_private.outbox(id),
  admin_id uuid not null references auth.users(id),
  previous_status text not null,
  decision text not null check (decision in ('sent','retry','suppress')),
  created_at timestamptz not null default now()
);
...
create table portal_private.worker_health (
  id boolean primary key default true check(id),
  last_worker_at timestamptz,
  last_sent_at timestamptz
);
insert into portal_private.worker_health(id) values (true);

alter table portal_private.teams enable row level security;
alter table portal_private.admin_memberships enable row level security;
alter table portal_private.subscriptions enable row level security;
alter table portal_private.outbox enable row level security;
alter table portal_private.rate_limits enable row level security;
alter table portal_private.mail_resolutions enable row level security;
alter table portal_private.worker_health enable row level security;
-- There are deliberately no anon/authenticated RLS policies. service_role is the
-- only DB caller and bypasses RLS; the Edge handler verifies admin authorization.
revoke all on all tables in schema portal_private from public, anon, authenticated;
grant select, insert, update, delete on all tables in schema portal_private to service_role;

-- SECURITY INVOKER: execute is explicitly denied to browsers below. This single
-- service RPC exposes atomic operations without exposing the private schema.
